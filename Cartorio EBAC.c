...
#include <locale.h> //biblioteca de alocações de texto por região "pra dizer q sou do BR"
#include <string.h> //biblioteca responsavel por cuidar da string

int registro()//função responsavel por cadastrar os usuários no sistema 
{
	//inicio criacão de variáveis/string
	char arquivo[40];
	char cpf[40];
	char nome[40];
	char sobrenome[40];
	char cargo[40];
	//final da criação de váraveis/string
	
	printf("Digite o CPF a ser cadastrado : "); //coletanto informação do usuário
	scanf("%s", cpf);// %s refere-se a string
	
	strcpy(arquivo, cpf); //responsanvael por copiar os valores da string
...
int main()
	{
	int opcao=0;//definino variaveis
	int laco=1;
